...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18',
                    ...
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet50)')
parser.add_argument('--output_dir', default='save_smd', type=str, metavar='PATH',
                    help='path to output dir')

parser.add_argument('-a_t', '--arch_t', metavar='ARCH_T', default='resnet50',
                    choices=model_names,
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet50)')
...
parser.add_argument('--times', default=1, type=float, help='times for distill loss')
parser.add_argument('--tau', type=float, default=0.04, help='tau for metric distillation')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # create model
    print("=> loading teacher model '{}'".format(args.arch_t))
    teacher_model = torchvision.models.__dict__[args.arch_t]()
    ...
    # load from pre-trained teacher, before DistributedDataParallel constructor
    if os.path.isfile(args.teacher_path):
        ...
        print("=> loaded pre-trained teacher model '{}'".format(args.teacher_path))
    else:
    ...
    criterion_kd = SMD(tau=args.tau).cuda(args.gpu)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, teacher_model, criterion, optimizer, epoch, args, criterion_kd)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            save_checkpoint({
                ...
                pre_filename=os.path.join(args.output_dir, 'checkpoint_{:04d}.pth.tar'.format(epoch - 1)))
...
def train(train_loader, model, teacher_model, criterion, optimizer, epoch, args, criterion_kd):
    batch_time = AverageMeter('Time', ':6.3f')
    data_time = AverageMeter('Data', ':6.3f')
    losses = AverageMeter('Loss', ':.4f')
    # ssl_losses = AverageMeter('ssl Loss', ':.4f')
    cos_sim_losses = AverageMeter('Cos Sim Loss', ':.4f')
    distill_losses = AverageMeter('Distill Loss', ':.4f')
    progress = ProgressMeter(
        len(train_loader),
        [batch_time, data_time, losses, cos_sim_losses, distill_losses],
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        # compute teacher output and distill loss
        with torch.no_grad():
            t_z1 = teacher_model(images[0])
            t_z2 = teacher_model(images[1])
        
        # compute output and loss
        z1 = model(images[0])
        z2 = model(images[1])
        
        cos_sim_loss = -(criterion(t_z1, z1).mean() + criterion(t_z2, z2).mean()) * 0.5
        
        distill_loss = (criterion_kd(t_z1, z1) + criterion_kd(t_z2, z2)) * 0.5
        
        if epoch < 2:
            loss = cos_sim_loss + args.times * distill_loss
        else:
            loss = distill_loss
        
        losses.update(loss.item(), images[0].size(0))
        # ssl_losses.update(ssl_loss.item(), images[0].size(0))
        cos_sim_losses.update(cos_sim_loss.item(), images[0].size(0))
        distill_losses.update(distill_loss.item(), images[0].size(0))
